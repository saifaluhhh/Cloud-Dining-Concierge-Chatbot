...
Amazon Lex: The user interacts with the chatbot via Lex, which handles Natural Language Processing (NLP) and intent recognition.
AWS Lambda: Lex triggers Lambda functions to process inputs, perform business logic (search restaurants, manage reservations), and communicate with DynamoDB.
Amazon DynamoDB: Stores user information, preferences, and restaurant details.
Amazon SNS (Optional): Send SMS notifications with restaurant details or booking confirmations.
External APIs (Optional): Integrates with restaurant listing services (Yelp, Zomato, etc.) for retrieving real-time restaurant data.
# Features
